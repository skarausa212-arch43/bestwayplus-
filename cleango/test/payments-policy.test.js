...
async function main() {
  ...
  try {
    ...
    // Dispatch honours the provider's city, so the fixture must book where the
    // seeded provider actually works — not a hard-coded Warsaw.
    const CITY = (await req('GET', '/api/me', { token: cl })).json.user.city;

    await ok('#3 payment gate: cleaner cannot go en route while the card is not captured', async () => {
      const bk = (await req('POST', '/api/bookings', { token: cust, body: { startNow: true, service: 'standard', rooms: 2, baths: 1, address: 'A', city: CITY } })).json.booking;
    ...
    await ok('#3 start is also gated: in_progress blocked until captured', async () => {
      const bk = (await req('POST', '/api/bookings', { token: cust, body: { startNow: true, service: 'standard', rooms: 1, baths: 1, address: 'C', city: CITY } })).json.booking;
    ...
    await ok('#2 refund: cancelling a captured booking before departure refunds + notifies the customer', async () => {
      const bk = (await req('POST', '/api/bookings', { token: cust, body: { startNow: true, service: 'standard', rooms: 2, baths: 1, address: 'B', city: CITY } })).json.booking;
      await req('POST', `/api/bookings/${bk.id}/accept`, { token: cl });
    ...
    await ok('#2 late cancel: after departure we withhold 40% and refund 60%', async () => {
      const bk = (await req('POST', '/api/bookings', { token: cust, body: { startNow: true, service: 'standard', rooms: 2, baths: 1, address: 'D', city: CITY } })).json.booking;
