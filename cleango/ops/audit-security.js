...
(async () => {
  // ── actors ──
  const cust = (await api('/api/register', 'POST', { email: `c${ts}@t.co`, password: 'Passw0rd!Long1', name: 'Client A', role: 'customer', phone: '+48500600801' })).json;
  const cust2 = (await api('/api/register', 'POST', { email: `c2${ts}@t.co`, password: 'Passw0rd!Long1', name: 'Client B', role: 'customer', phone: '+48500600802' })).json;
  ...
  await ok('Блокировка: админ блокирует клиента, тот не может войти', async () => {
    ...
    victim = (await api('/api/register', 'POST', { email: em, password: 'Passw0rd!Long1', name: 'Victim', role: 'customer', phone: '+48500600809' }, null)).json;
  ...
  await ok('Удаление аккаунта (GDPR): PII анонимизируется, сессия отзывается', async () => {
    ...
    const u = (await api('/api/register', 'POST', { email: em, password: 'Passw0rd!Long1', name: 'To Delete', role: 'customer', phone: '+48500600810' }, null)).json;
  ...
  await ok('Удаление с активным заказом блокируется (409)', async () => {
    ...
    const u = (await api('/api/register', 'POST', { email: em, password: 'Passw0rd!Long1', name: 'Busy', role: 'customer', phone: '+48500600811' }, null)).json;
