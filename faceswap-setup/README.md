...
- git; ffmpeg ставится автоматически pip-пакетом `ffmpeg-binaries`
- Системный tkinter (`sudo apt install python3.X-tk` под вашу версию Python) — скрипт ставит его сам; он нужен даже для консольного режима
- ~2–6 ГБ диска под зависимости (PyTorch, Keras 3, OpenCV); CPU-сборка torch компактнее CUDA-сборки
