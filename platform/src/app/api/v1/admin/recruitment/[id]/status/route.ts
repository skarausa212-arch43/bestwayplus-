...
import { updateStatus } from '@/modules/recruitment/service';
import { apiError, handleApiError } from '@/lib/api';

export const dynamic = 'force-dynamic';

const body = z.object({ status: z.enum(['OPEN', 'IN_PROGRESS', 'CLOSED', 'CANCELLED']) });
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await updateStatus(session, id, status, await requestContext());
