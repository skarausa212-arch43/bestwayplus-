...
import { getInvestorForStaff } from '@/modules/admin/investors';
...
export default async function AdminInvestorDetail({
  ...
  const investor = await getInvestorForStaff(session, id);
  ...
  const name = investor.displayName || investor.user.email;

  const PROFILE_ROWS: Array<[string, string]> = [
    [f('entityType'), investor.entityType === 'COMPANY' ? f('company') : f('individual')],
    [f('displayName'), investor.displayName ?? '—'],
    [f('company'), investor.company ?? '—'],
    [f('country'), investor.country ?? '—'],
    [f('businessArea'), investor.businessArea ?? '—'],
    [f('investmentRange'), investor.investmentRange ?? '—'],
  ];
    ...
    <div className="grid gap-6">
      <div className="flex flex-wrap items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-1.5 text-sm text-ink-muted">
            {[investor.company, investor.country].filter(Boolean).join(' · ')}
          </p>
          <p className="mt-1 text-xs text-ink-faint">
            {investor.user.email} · {investor.user.responsibleManager?.email ?? '—'}
          </p>
        </div>
        <Pill tone={TONE[investor.verificationStatus] ?? 'neutral'}>{ts(`ver${investor.verificationStatus}`)}</Pill>
      </div>
      ...
      <nav className="flex flex-wrap gap-1 border-b border-line-faint pb-2">
        {TABS.map((name) => (
          <a
            key={name}
            href={`/admin/investors/${id}?tab=${name}`}
        ...
        <div className="grid gap-4">
          {active === 'profile' && (
            <Card>
              ...
              {(investor.investmentInterests || investor.footballInterests || investor.professionalBackground) && (
                <div className="mt-6 grid gap-4 border-t border-line-faint pt-5 sm:grid-cols-3">
                  {investor.investmentInterests && (
                    <div>
                      <dt className="font-display text-[9.5px] font-semibold uppercase tracking-[0.2em] text-ink-faint">{f('investmentInterests')}</dt>
                      <dd className="mt-1.5 whitespace-pre-line text-sm text-ink-muted">{investor.investmentInterests}</dd>
                    </div>
                  )}
                  {investor.footballInterests && (
                    <div>
                      <dt className="font-display text-[9.5px] font-semibold uppercase tracking-[0.2em] text-ink-faint">{f('footballInterests')}</dt>
                      <dd className="mt-1.5 whitespace-pre-line text-sm text-ink-muted">{investor.footballInterests}</dd>
                    </div>
                  )}
                  {investor.professionalBackground && (
                    <div>
                      <dt className="font-display text-[9.5px] font-semibold uppercase tracking-[0.2em] text-ink-faint">{f('professionalBackground')}</dt>
                      <dd className="mt-1.5 whitespace-pre-line text-sm text-ink-muted">{investor.professionalBackground}</dd>
                    </div>
                  )}
                </div>
              )}
