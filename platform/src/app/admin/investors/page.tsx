...
import { listInvestors } from '@/modules/admin/investors';
...
export default async function AdminInvestors({
  ...
  const investors = await listInvestors(session, {
  ...
  return (
    <div className="grid gap-6">
      <SectionTitle>{t('investors')}</SectionTitle>
      ...
      {investors.length === 0 ? (
        ...
        <ul className="grid gap-3">
          {investors.map((investor) => (
            <Card as="li" key={investor.id}>
              <div className="flex flex-wrap items-start justify-between gap-5">
                <div className="min-w-0">
                  <a href={`/admin/investors/${investor.userId}`} className="font-display text-base font-extrabold text-ink hover:text-emerald">
                    {investor.displayName || investor.user.email}
                  </a>
                  <p className="mt-1 text-xs text-ink-muted">
                    {[investor.company, investor.country].filter(Boolean).join(' · ')}
                  </p>
                  <p className="mt-2 text-xs text-ink-faint">
                    {format.dateTime(investor.user.createdAt, { dateStyle: 'medium' })}
                  </p>
                </div>
