...
import { listClubs } from '@/modules/admin/clubs';
...
export default async function AdminClubs({
  ...
  const clubs = await listClubs(session, {
  ...
  return (
    <div className="grid gap-6">
      <SectionTitle>{t('clubs')}</SectionTitle>
      ...
      {clubs.length === 0 ? (
        ...
        <ul className="grid gap-3">
          {clubs.map((club) => (
            <Card as="li" key={club.id}>
              <div className="flex flex-wrap items-start justify-between gap-5">
                <div className="min-w-0">
                  <a href={`/admin/clubs/${club.userId}`} className="font-display text-base font-extrabold text-ink hover:text-emerald">
                    {club.clubName || [club.firstName, club.lastName].filter(Boolean).join(' ') || club.user.email}
                  </a>
                  <p className="mt-1 text-xs text-ink-muted">
                    {[club.league, club.country].filter(Boolean).join(' · ')}
                  </p>
                  <p className="mt-2 text-xs text-ink-faint">
                    {format.dateTime(club.user.createdAt, { dateStyle: 'medium' })}
                  </p>
                </div>
