...
Двойной клик по `AndroidEmulator.exe` в корне папки. Больше ничего заранее
делать не нужно.

**Первый запуск** займёт несколько минут: в архиве нет `node_modules`, поэтому
exe сам поставит зависимости и скачает Chromium (~150 МБ). Он делает это в
отдельном чёрном окне консоли — специально видимом, чтобы приложение не
выглядело зависшим. Окно закроется само. Дальнейшие запуски — мгновенные.

Затем открывается окно приложения: без адресной строки, вкладок и консоли.
Закрыли окно — всё остановилось, включая запущенные устройства.

Единственное требование — установленный Node.js 20+ (https://nodejs.org).
Если его нет, exe скажет об этом окном с текстом.
...
Если что-то пойдёт не так, вы увидите окно с описанием — **вместе с тем, что
сказал сам сервер**, а не только «не запустилось». Полный лог лежит в
`panel.log` рядом с exe.
