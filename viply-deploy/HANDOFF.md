...
## Мульти-почта: ключевые фичи

Вкладки Inbox/Spam/Sent/Trash (+«Not spam»/«Spam»/Restore), поиск по всем ящикам,
фильтр непрочитанных, «прочитать все», Load more, свайпы (влево удалить / вправо спам),
массовое выделение (тап по аватарке) и удаление в Trash (из Trash — навсегда),
вложения (скачивание и отправка до 18 МБ), Reply all / Forward, Sign out,
смена пароля (🔑) и восстановление по recovery email (ссылка с лендинга,
письма шлёт postmaster — POSTMASTER_PASS в .env), PWA (manifest+sw, ставится
на экран «Домой»), «📦 Combo account» — сборка: все ящики сессии под
именем+паролем (scrypt, /data/bundles.json), новые ящики синкаются в сборку.

## Админ и мониторинг

ADMIN_EMAIL=romanby@домен (пишется установщиком в .env). Сессия/телеграм-чат
с этим ящиком видит: кнопку «👑 Admin» в /multi/ и команду /stats в боте
(статистика читается из конфига DMS, смонтирован ro в /dmsconfig).
Мониторинг: /usr/local/bin/emailinc-monitor.sh (cron */10) — контейнеры,
срок сертификата, диск; алерты в телеграм-чат админа (admin-chat.json пишется
ботом, когда админ подключает свой ящик). Квоты: 1G на ящик при регистрации
(ENABLE_QUOTAS=1 + dovecot-quotas.cf).
